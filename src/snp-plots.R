...
ggsave("/media/cdc_documents/Abstracts/ASM2018/poster-figs/20180430-cluster-SNPs-02.pdf", device = 'pdf', width = 8, height = 4, units = 'in', useDingbats=F)
head(clust.wtf)
...
rev=sort(unique(clust.years$V3), decreasing =T)
...
(ggplot( data = clust.years2kp, aes(x=as.factor(V2), y=V3))
  + geom_jitter(width = 0.1, height=0.35, size=1, color="red", alpha=0.6)
  ...
  + labs(y="Colinear cluster", x="Year of Isolation")
  #+ geom_vline( aes(xintercept=2009), lty=2, size=0.25 )
  + theme(panel.grid.minor.x = element_blank() )# ,axis.text.x=element_text(angle=0, hjust=0))
...
?geom_density
(ggplot( data = wtf, aes(x=V2, fill=V3, color=V3, alpha=0.5))
...
pairwise.snps <- read.table("../../kSNP/20180501-SNP-dist/20180501-pairwiseSNPs-down.tsv",sep="\t",header=F);
head(pairwise.snps)
...
ggsave("../../kSNP/20180501-SNP-dist/20180501-pairwiseSNPs-down.pdf", device = 'pdf', width = 8, height = 6, units = 'in', useDingbats=F)
...
(ggplot( data = pairwise.pert, aes(x=V1, y=V2))
  ...
  + geom_boxplot(outlier.size = 0, outlier.stroke = 0, alpha=0)
  + theme_minimal()
#  + xlim("Bparapertussis","Bpertussis")
#  + ylim(0,2000)
  + labs(x="Species", y="Pairwise SNP distance")
  + scale_y_continuous( labels = comma )#, limits=c(0,500))
  + theme( panel.grid.major.x = element_blank() )
)
ggsave("../../kSNP/20180501-SNP-dist/20180501-pairwiseSNPs-Bp.pdf", device = 'pdf', width = 4, height = 4, units = 'in', useDingbats=F)
